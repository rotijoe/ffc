...
const BUSINESS_TYPE_ID = 7844; // FSA's ID for "Takeaway/sandwich shop"
...
async function fetchAllForAuthority(authorityId: number) {
  ...
  while (true) {
    ...
    const establishments = data.establishments as any[];
    ...
    console.log(`Found ${establishments.length} establishments for authority ${authorityId}.`);
...
function mapToDbRow(est: any, syncTimestamp: string) {
  return {
    fhrs_id: est.FHRSID,
    ...
    address: [est.AddressLine1, est.AddressLine2, est.AddressLine3, est.AddressLine4].filter(Boolean).join(', '),
...
// Deno.serve is the recommended way to start the server for Supabase Edge Functions.
Deno.serve(async (_req: Request) => {
  const syncTimestamp = new Date().toISOString(); // 1. Record the start time.
  console.log(`Starting sync at: ${syncTimestamp}`);

  try {
    // These environment variables are automatically provided by the Supabase runtime.
    ...
    const filteredResults = allResults.filter(est => desiredBusinessTypes.includes(est.BusinessTypeID));

    if (filteredResults.length === 0) {
      return new Response(JSON.stringify({ message: "No relevant chicken shops found to update." }), {
        status: 200,
    ...
    const chickenShops = filteredResults.map(est => mapToDbRow(est, syncTimestamp)); // Pass the timestamp to the mapper.
    ...
    return new Response(JSON.stringify({ upserted_count: chickenShops.length }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err) {
